...
pub struct AppConfig {
    ...
    #[serde(default = "default_schedule_quote")]
    pub schedule_quote: String,
...
impl NoteStore {
    ...
    pub fn list_notes(&self) -> Result<Vec<NoteMetadata>, AppError> {
        self.list_notes_filtered(None)
    }

    pub fn list_notes_by_kind(&self, kind: &str) -> Result<Vec<NoteMetadata>, AppError> {
        self.list_notes_filtered(Some(kind))
    }

    pub fn find_note_by_date(&self, date: &str, kind: &str) -> Result<Option<NoteMetadata>, AppError> {
        self.ensure_storage()?;
        let metadata = self.load_metadata()?.notes;
        Ok(metadata
            .into_iter()
            .find(|note| note.kind == kind && note.schedule_date.as_deref() == Some(date)))
    }

    pub fn list_schedule_dates(&self) -> Result<Vec<String>, AppError> {
        self.ensure_storage()?;
        let metadata = self.load_metadata()?.notes;
        let mut dates: Vec<String> = metadata
            .into_iter()
            .filter(|note| note.kind == "schedule" && note.schedule_date.is_some())
            .filter(|note| {
                // 只标记有实际内容的日程
                let content = self.read_note_content(&note.file_name, &note.category);
                content.map(|c| c.trim().len() > 0).unwrap_or(false)
            })
            .filter_map(|note| note.schedule_date)
            .collect();
        dates.sort();
        dates.dedup();
        Ok(dates)
    ...
    fn read_note_content(&self, file_name: &str, category: &str) -> Result<String, AppError> {
        let path = self.note_path_in_category(file_name, category);
        Ok(fs::read_to_string(path)?)
    }

    fn list_notes_filtered(&self, kind: Option<&str>) -> Result<Vec<NoteMetadata>, AppError> {
        self.ensure_storage()?;
        ...
        if let Some(k) = kind {
            metadata.retain(|note| note.kind == k);
        }
    ...
    pub fn read_note(&self, id: &str) -> Result<Note, AppError> {
        ...
        Ok(Note {
            ...
            kind: metadata.kind,
            schedule_date: metadata.schedule_date.clone(),
    ...
    pub fn create_note(&self, request: SaveNoteRequest) -> Result<Note, AppError> {
        ...
        let schedule_date = request.schedule_date.clone();
    ...
    pub fn update_note(&self, id: &str, request: SaveNoteRequest) -> Result<Note, AppError> {
        ...
        note.schedule_date = request.schedule_date.clone();
        note.updated_at = now;
        ...
        let result = Note {
            ...
            schedule_date: note.schedule_date.clone(),
    ...
    fn default_config(&self) -> AppConfig {
        AppConfig {
            ...
            schedule_quote: default_schedule_quote(),
            notes_dir: None,
    ...
    fn ensure_storage(&self) -> Result<(), AppError> {
        ...
        if !self.metadata_path().exists() {
            ...
            if self.migrate_schedule_dates(&mut metadata)? {
                self.save_metadata(&metadata)?;
            }
        }
        Ok(())
    ...
    fn migrate_schedule_dates(&self, metadata: &mut MetadataFile) -> Result<bool, AppError> {
        let mut changed = false;
        for note in &mut metadata.notes {
            if note.kind == "schedule" && note.schedule_date.is_none() {
                note.schedule_date = Some(note.created_at.format("%Y-%m-%d").to_string());
                changed = true;
            }
        }
        Ok(changed)
...
fn default_schedule_quote() -> String {
    "千里之行，始于足下。".into()
}
