...
#[tauri::command]
fn notes_find_by_date(date: String, kind: String) -> Result<Option<NoteMetadata>, AppError> {
    default_store()?.find_note_by_date(&date, &kind)
}

#[tauri::command]
fn schedule_dates_list() -> Result<Vec<String>, AppError> {
    default_store()?.list_schedule_dates()
}
