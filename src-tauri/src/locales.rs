...
pub fn notepad_window_title(locale: Locale) -> &'static str {
    match locale {
        Locale::ZhCn => "有记·三得随心记",
        Locale::EnUs => "YouNote Free Note",
        Locale::ZhHk => "有記·三得隨心記",
    }
}
...
mod tests {
    ...
    fn localizes_native_shell_strings_for_supported_locales() {
        assert_eq!(app_name(Locale::ZhCn), "有记·三得");
        assert_eq!(app_name(Locale::EnUs), "YouNote");
        assert_eq!(app_name(Locale::ZhHk), "有記·三得");

        assert_eq!(
            notepad_window_title(Locale::EnUs),
            "YouNote Free Note"
        );
        assert_eq!(tile_window_title(Locale::ZhHk), "有記·三得磁貼");
        assert_eq!(tray_tooltip(Locale::EnUs), "YouNote");
        assert_eq!(tray_show_main_label(Locale::EnUs), "Open Main Window");
        assert_eq!(tray_quick_note_label(Locale::ZhHk), "隨心記");
        assert_eq!(
    ...
    fn localizes_macos_native_menu_strings_for_supported_locales() {
        ...
        assert_eq!(macos_menu_about_label(Locale::ZhCn), "关于有记·三得");
        assert_eq!(
            macos_menu_about_label(Locale::EnUs),
            "About YouNote"
        );
        assert_eq!(macos_menu_services_label(Locale::ZhHk), "服務");
        assert_eq!(macos_menu_hide_app_label(Locale::ZhCn), "隐藏有记·三得");
