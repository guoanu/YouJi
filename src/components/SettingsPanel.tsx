  ...
  return (
    <aside className="w-[360px] h-full shrink-0 border-l border-paper-deep/30 bg-cloud/92 backdrop-blur-sm flex flex-col">
      ...
      <div className="flex-1 overflow-y-auto scrollbar-hidden px-4 py-4 space-y-5">
        ...
        <section className="space-y-2">
          <label className="block text-[11px] font-body text-ink-faint">
            {t("settings.scheduleQuote.label", { defaultValue: "在日程下方添加语句：" })}
          </label>
          <input
            type="text"
            value={config.scheduleQuote ?? ""}
            onChange={(event) => setConfigValue("scheduleQuote", event.target.value)}
            placeholder={t("settings.scheduleQuote.placeholder", {
              defaultValue: "显示在日程编辑栏底部",
            })}
            className="w-full h-9 rounded-lg px-2.5 text-[12px] font-body text-ink bg-paper-warm/45 border border-paper-deep/25 focus:border-bamboo/30 placeholder:text-ink-ghost/50"
          />
        </section>
