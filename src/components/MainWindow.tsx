...
import type { CategoryGroup, FilteredResult } from "../features/notes/noteUtils";
import {
...
export function MainWindow({
  ...
  const [freeNotes, setFreeNotes] = useState<NoteMetadata[]>([]);
  const [schedules, setSchedules] = useState<NoteMetadata[]>([]);
  const [notesExpanded, setNotesExpanded] = useState(false);
  const [freeNotesExpanded, setFreeNotesExpanded] = useState(false);
  ...
  const scheduleCount = useMemo(
    () => schedules.filter((s) => s.preview.trim().length > 0 || s.title.trim().length > 0).length,
    [schedules],
  );
  ...
  const selectedKind = useMemo(() => {
    ...
    if (externalFiles.some((f) => f.id === selectedId)) return "external";
    if (freeNotes.some((n) => n.id === selectedId)) return "freeNote";
    if (schedules.some((n) => n.id === selectedId)) return "schedule";
    if (notes.some((n) => n.id === selectedId)) return "note";
    return null;
  }, [selectedId, notes, freeNotes, schedules, externalFiles]);

  const kindBadge = useMemo(() => {
    switch (selectedKind) {
      case "note":
        return { label: t("main.section.notes", { defaultValue: "笔记" }), color: "bg-bamboo/12 text-bamboo border-bamboo/25" };
      case "freeNote":
        return { label: t("main.section.freeNotes", { defaultValue: "随心记" }), color: "bg-blue-400/12 text-blue-500 border-blue-400/25" };
      case "schedule":
        return { label: t("main.section.schedules", { defaultValue: "日程" }), color: "bg-amber-400/12 text-amber-500 border-amber-400/25" };
      case "external":
        return { label: t("main.externalFiles.title", { defaultValue: "外部文件" }), color: "bg-purple-400/12 text-purple-500 border-purple-400/25" };
      default:
        return null;
    }
  }, [selectedKind, t]);
  ...
  const noteMenuTarget = useMemo(
    () =>
      notes.find((note) => note.id === noteMenu?.noteId) ??
      freeNotes.find((note) => note.id === noteMenu?.noteId) ??
      schedules.find((note) => note.id === noteMenu?.noteId) ??
      null,
    [noteMenu?.noteId, notes, freeNotes, schedules],
  );
  ...
  const filtered = useMemo<FilteredResult>(
    () => filterAll(notes, freeNotes, schedules, searchQuery),
    [notes, freeNotes, schedules, searchQuery],
  );

  const categoryGroups = useMemo(
    () => groupNotesByCategory(filtered.notes, categories),
    [filtered.notes, categories],
  );
  ...
  const replaceNoteMetadata = useCallback((note: Note) => {
    ...
    const updateList = (current: NoteMetadata[]) => {
    ...
    if (note.kind === "freeNote") {
      setFreeNotes(updateList);
    } else if (note.kind === "schedule") {
      setSchedules(updateList);
    } else {
      setNotes(updateList);
    }
  }, []);
  ...
  const refreshNotes = useCallback(async () => {
    const [loadedNotes, loadedFreeNotes, loadedSchedules, loadedCategories] = await Promise.all([
      listNotesByKind("note"),
      listNotesByKind("freeNote"),
      listNotesByKind("schedule"),
      listCategories(),
    ]);
    setNotes(loadedNotes);
    setFreeNotes(loadedFreeNotes);
  ...
  useEffect(() => {
    ...
    async function bootstrap() {
      setIsLoading(true);
      try {
        const [loadedConfig, loadedNotes, loadedFreeNotes, loadedSchedules, loadedCategories] =
          await Promise.all([
            getConfig(),
            listNotesByKind("note"),
            listNotesByKind("freeNote"),
  ...
  const performSave = useCallback(
    async (force: boolean): Promise<boolean> => {
      ...
      try {
        ...
        } else if (id.startsWith("pending-schedule-")) {
          // 虚拟日程占位 → 首次保存时真正创建笔记
          const scheduleDate = pendingScheduleDateRef.current ?? "";
  ...
  const handleNewNote = async () => {
    ...
    await createNoteInCategory(activeCategory);
  };

  const handleConfirmNewNote = async (category: string) => {
    setShowCategoryPicker(false);
    await createNoteInCategory(category);
  };

  const createNoteInCategory = async (category: string) => {
    try {
      const note = await createNote({ title: "", content: "", category, kind: "note" });
      replaceNoteMetadata(note);
      applyNote(note);
    } catch (error) {
      showToast(getErrorMessage(error));
  ...
  const handleOpenCalendar = async () => {
    try {
      const dates = await listScheduleDates();
  ...
  const handleDeleteScheduleDate = async (date: string) => {
    try {
      const existing = await findNoteByDate(date, "schedule");
      if (existing) {
  ...
  const handleSelectCalendarDate = async (date: string) => {
    ...
    try {
      ...
      if (existing) {
        ...
        // 使用虚拟 ID 占位，直到用户保存时才真正创建笔记
        const fakeId = `pending-schedule-${date}`;
        pendingScheduleDateRef.current = date;
        loadEpoch.bump();
        selectedIdRef.current = fakeId;
        titleValueRef.current = "";
        contentValueRef.current = "";
        saveStateRef.current = "dirty";
    ...
    <div className="w-full h-screen flex flex-col">
      <div className="relative noise-bg bg-cloud overflow-hidden flex flex-col flex-1">
        ...
        >
          <div className="flex items-center gap-3 min-w-0">
            ...
            {kindBadge && (
              <span
                className={`shrink-0 text-[9px] font-body leading-none px-1.5 py-0.5 rounded-full border ${kindBadge.color}`}
              >
                {kindBadge.label}
              </span>
            )}
            ...
            <div className="flex flex-col h-full" style={{ width: `${sidebarWidth}px` }}>
              ...
              <div className="px-3 pb-2 shrink-0 space-y-1">
                ...
                >
                  ...
                  <span>{t("main.sidebar.newFreeNote", { defaultValue: "新建随心记" })}</span>
                  <kbd className="ml-auto text-[10px] text-ink-ghost/50 font-mono bg-paper-warm/60 px-1 py-0.5 rounded">
                    Ctrl+Space
                  </kbd>
                </button>
                <button
              ...
              <div className="flex-1 overflow-y-auto px-2 pb-2">
                <div className="space-y-0.5">
                  ...
                  <div className="mb-0.5">
                    <div
                      className={`flex items-center gap-1.5 px-2.5 py-1.5 rounded-lg cursor-pointer select-none transition-all duration-200 border border-bamboo/15 ${
                        selectedKind === "note" ? "bg-bamboo/8" : "bg-transparent"
                      }`}
                    ...
                    >
                      <svg
                        ...
                        strokeLinecap="round"
                        strokeLinejoin="round"
                        className={`text-bamboo/50 shrink-0 transition-transform duration-200 ${notesExpanded ? "rotate-90" : ""}`}
                      >
                      ...
                      <span className="text-[11px] text-bamboo/70 font-medium truncate">
                        {t("main.section.notes", { defaultValue: "笔记" })}
                      </span>
                      ...
                      <button
                        ...
                        className="text-[10px] text-bamboo/40 hover:text-bamboo transition-colors cursor-pointer"
                              ...
                              return (
                                ...
                                >
                                  ...
                                  <div className="flex items-baseline justify-between mb-0.5">
                                    <span className={`text-[13px] font-display font-medium truncate pr-2 transition-colors flex items-center gap-1.5 ${isSelected ? "text-bamboo" : "text-ink-soft"}`}>
                                    ...
                                    <button
                                      onClick={(e) => { e.stopPropagation(); handleRemoveExternalFile(file.id); }}
                  ...
                  {/* Section 2: Free Notes */}
                  <div className="mb-0.5 pt-1">
                    <div
                      className={`flex items-center gap-1.5 px-2.5 py-1.5 rounded-lg cursor-pointer select-none transition-all duration-200 border border-bamboo/15 ${
                        selectedKind === "freeNote" ? "bg-bamboo/8" : "bg-transparent"
                      }`}
                      onClick={() => setFreeNotesExpanded(!freeNotesExpanded)}
                    >
                      <svg
                        width="10"
                        height="10"
                        ...
                        strokeLinecap="round"
                        strokeLinejoin="round"
                        className={`text-bamboo/50 shrink-0 transition-transform duration-200 ${freeNotesExpanded ? "rotate-90" : ""}`}
                      >
                        <polyline points="9 18 15 12 9 6" />
                      </svg>
                      <svg
                      ...
                      <span className="text-[11px] text-bamboo/70 font-medium truncate">
                        {t("main.section.freeNotes", { defaultValue: "随心记" })}
                      </span>
                      <span className="text-[9px] text-bamboo/40 font-mono ml-auto shrink-0">
                        {filtered.freeNotes.length}
                      </span>
                    </div>
                    {freeNotesExpanded && (
                      <div className="bg-bamboo/[0.03] border border-t-0 border-bamboo/10 rounded-b-lg pb-1 pt-1">
                        {filtered.freeNotes.length === 0 ? (
                          <div className="px-3 py-3 text-center text-[11px] text-ink-ghost/50">
                            {searchQuery
                              ? t("main.search.noFreeNotes", { defaultValue: "没有匹配的随心记" })
                              : t("main.section.noFreeNotes", { defaultValue: "还没有随心记，点击上方按钮创建" })}
                          </div>
                        ) : (
                          filtered.freeNotes.map((note) => {
                            const isSelected = note.id === selectedId;
                            const isHovered = note.id === hoveredId;
                            return (
                              <div
                                ...
                                onMouseEnter={() => setHoveredId(note.id)}
                                onMouseLeave={() => setHoveredId(null)}
                                className={`w-full text-left rounded-lg px-2.5 py-2 mx-1 transition-all duration-[600ms] cursor-pointer group relative ${
                                  isSelected ? "bg-bamboo-mist/70" : isHovered ? "bg-paper-warm/70" : "bg-transparent"
                                }`}
                                style={{ width: "calc(100% - 8px)" }}
                              >
                                ...
                                />
                                <div className="flex items-baseline justify-between mb-0.5">
                                  <span className={`text-[13px] font-display font-medium truncate pr-2 transition-colors ${isSelected ? "text-bamboo" : "text-ink-soft"}`}>
                                    {getDisplayTitle(note, t)}
                                  </span>
                                  <span className="text-[10px] text-ink-ghost font-mono tabular-nums shrink-0">
                                    {formatShortDate(note.updatedAt)}
                  ...
                  {/* Section 3: Schedules — opens calendar */}
                  <div className="mb-0.5 pt-1">
                    <div
                      className={`flex items-center gap-1.5 px-2.5 py-1.5 rounded-lg cursor-pointer select-none transition-all duration-200 border border-bamboo/15 hover:bg-bamboo/[0.04] ${
                        selectedKind === "schedule" ? "bg-bamboo/8" : "bg-transparent"
                      }`}
                    ...
                    >
                      ...
                      <span className="text-[11px] text-bamboo/70 font-medium truncate">
                        {t("main.section.schedules", { defaultValue: "工作日日程" })}
                      </span>
                      <span className="text-[9px] text-bamboo/40 font-mono ml-auto shrink-0">
                        {scheduleCount}
                      </span>
                  ...
                  {!isLoading &&
                    ...
                    <div className="px-3 py-8 text-center text-[12px] text-ink-ghost leading-relaxed">
                      {searchQuery
                        ? t("main.search.noResults", { defaultValue: "没有匹配的内容" })
                        : t("main.search.empty", { defaultValue: "还没有内容" })}
                    </div>
                  )}
                </div>
              </div>
          ...
          <div className="flex-1 flex flex-col min-w-0">
            ...
            {!selectedId && !isLoading ? (
              <div className="flex-1 flex flex-col items-center justify-center select-none">
                <span
                  className="font-serif italic font-medium leading-none tracking-tight"
                  style={{
                    fontSize: "clamp(28px, 5vw, 56px)",
                    background: "linear-gradient(135deg, #b8a99a 0%, #d4c8b8 40%, #a89880 100%)",
            ...
            {selectedKind === "schedule" && (
              <div className="flex items-center justify-center px-4 h-6 border-t border-paper-deep/15 bg-amber-50/30 shrink-0 select-none">
                <span
                  className="text-amber-600/50 font-serif tracking-wide italic"
                  style={{ fontSize: `${Math.max(10, (settingsConfig?.fontSize ?? 14) - 2)}px` }}
                >
                  {settingsConfig?.scheduleQuote || "千里之行，始于足下。"}
                </span>
              </div>
            )}
      ...
      {showCalendar && (
        <CalendarPicker
          ...
          onSelectDate={(date) => void handleSelectCalendarDate(date)}
          onDeleteDate={(date) => void handleDeleteScheduleDate(date)}
      ...
      {showCategoryPicker && (
        ...
        >
          <div
            className="bg-cloud border border-paper-deep/50 rounded-xl shadow-lg py-2 px-1 min-w-[180px] max-w-[260px] max-h-[60vh] overflow-y-auto animate-menu-enter"
            onClick={(e) => e.stopPropagation()}
          >
            <div className="px-3 py-1.5 text-[10px] text-ink-ghost/50 font-mono tracking-wider uppercase">
              {t("main.category.selectPrompt", { defaultValue: "选择分类" })}
            </div>
            <button
              onClick={() => void handleConfirmNewNote("")}
              className="w-full text-left px-3 py-2 text-[12px] font-body text-ink-soft hover:bg-bamboo-mist/60 hover:text-bamboo rounded-lg transition-colors cursor-pointer"
            >
            ...
            {categories.map((cat) => (
              <button
                key={cat}
                onClick={() => void handleConfirmNewNote(cat)}
                className="w-full text-left px-3 py-2 text-[12px] font-body text-ink-soft hover:bg-bamboo-mist/60 hover:text-bamboo rounded-lg transition-colors cursor-pointer flex items-center gap-2"
              >
                <svg
                  width="11"
                  height="11"
                  ...
                  strokeLinecap="round"
                  strokeLinejoin="round"
                  className="text-bamboo/40 shrink-0"
                >
                  <path d="M22 19a2 2 0 0 1-2 2H4a2 2 0 0 1-2-2V5a2 2 0 0 1 2-2h5l2 3h9a2 2 0 0 1 2 2z" />
                </svg>
                {cat}
