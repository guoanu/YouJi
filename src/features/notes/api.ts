...
import type { ContentKind, Note, NoteMetadata, SaveNoteRequest } from "./types";
...
export function listNotesByKind(kind: ContentKind): Promise<NoteMetadata[]> {
  return invoke("notes_list_by_kind", { kind });
}

export function findNoteByDate(date: string, kind: ContentKind): Promise<NoteMetadata | null> {
  return invoke("notes_find_by_date", { date, kind });
}

export function listScheduleDates(): Promise<string[]> {
  return invoke("schedule_dates_list");
}
